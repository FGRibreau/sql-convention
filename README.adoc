...
functions and `+public+` functions - *notify*, format:
notify[_SchemaName_][_TableName_][_Event_]
(e.g. `+notifyAuthenticationUserCreated(user_id)+`): should only format
the notification message underneath and use pg_notify. Beware of the
...
alternative. - *private*, format: _[_functionName_]
(e.g. `+_resetFailedLogin+`): must never be exposed through the public
schema. Used mainly for consistency and business-rules - *public*,
format [_functionName_] (e.g. `+logIn(email, password)+`): must be
