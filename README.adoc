...
* `SET search_path=pg_catalog` to force to explicitely specify schema names in every object declaration (besides triggers). This will lower bugs and gives better understanding to developers because [explicit > implicit](https://getnobullshit.com).
...
Enum types should be in _singular_, in *snake_case**.
